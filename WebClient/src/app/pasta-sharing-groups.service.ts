...
export class PastaSharingGroupsService {
  ...
  addPastaGroupSharing(
    pastagroup: PastaGroupSharingRequest
  ): Observable<string> {
  ...
  getPastaGroupSharing(
    pastaBindID: string | null
  ): Observable<PastaGroupSharingResponse[]> {
    return this.httpClient.get<PastaGroupSharingResponse[]>(
      this.startURL + '/get/' + pastaBindID + '/' + this.authservice.getUserID()
    );
  }
}
